...
from feniQS.inference.forward_model import FEMU_F_KEYS
class JacobianFEMU_F_Dict(CallToGetUpdatedAttributes):
    def __init__(self, pp_jac, res_reaction_ids_groups, N, _relative=False):
        """
        The jacobian is put/computed in the attribute "self._jac_dict" which is a dictionary of dictionaries.
        The keys of the first dictionary are regarding outputs: FEMU_F_KEYS.OUT
        The keys of EVERY second dictionary are regarding inputs: FEMU_F_KEYS.INP
        Every value of the second dictionary is a np.array for the respective jacobian part (concerning the corresponding keys).
        N:
            A matrix which applies (at each checkpoint in the same way):
                measured_us = N @ imposed_us
                , which means, N is the derivative of measured_us w.r.t. imposed_us.
        _relative:
            If True, measured_us are considered in a 'relative' manner: relative to its very first item (at very first checkpoint).
            This means, the number of measured_us sets becomes 'one' smaller than the number of imposed_us
                , since the relative value of the very first item in measured_us would be ZERO and thus, useless.
            The Jacobian is adjusted in this case: we have a block column at the very first with all blocks being equal to (-N).
        """
        assert type(pp_jac)==PropagateJacobian
        assert type(pp_jac.gs[0]) == Residual_GDM
            ##### FOR NOW, IT IS HARD CODED.
            ##### The only allowable "g" (the forward-model output) which is consistent with FEMU-F forward model, is "Residual" (must be in Dgs_dps_checked[0]) .
        for l in res_reaction_ids_groups:
            for i in l:
                assert(i < len(pp_jac.dof.res))
        
        super().__init__(obj=pp_jac, attributes_list=['J_Fu_global', 'Dgs_dps_checked']) # results in: self.obj = pp_jac        
        self.res_reaction_ids_groups = res_reaction_ids_groups # for now, for one checkpoint
        self.N = N
        self.n_u_m, self.n_u_ad = self.N.shape
        self.n_p = len(pp_jac.dF_dps.keys())
        self._relative = _relative
        self._initiate_jac_dict()
    
    def _initiate_jac_dict(self):
        self._jac_dict = dict()
        for k_out in FEMU_F_KEYS.OUT:
            j2 = dict()
            for k_int in FEMU_F_KEYS.INP:
                j2.update({k_int: None}) # initiate to None
            self._jac_dict.update({k_out: j2})
        
    def __call__(self):
        J_Fu_global, Dgs_dps_checked = super().__call__() # J_Fu_global is a block array
        
        ##### The operations needed to be done only once (at the first call)
        if self._jac_dict[FEMU_F_KEYS.out_us][FEMU_F_KEYS.pars] is None: # if one of constant terms still not assigned to.
            self.n_ch = len(self.obj.checkpoints)
            self.n_ch_m = self.n_ch-1 if self._relative else self.n_ch # number of checkpoints corresponding to measurements (output)
            self.n_res, n_u_ad = J_Fu_global[0][0].shape
            assert (n_u_ad == self.n_u_ad)
            ### CONSTANT TERMs
            # D_u_m / d_pars, which is entirely ZERO.
            self._jac_dict[FEMU_F_KEYS.out_us][FEMU_F_KEYS.pars] = np.zeros((self.n_ch_m * self.n_u_m, self.n_p))
            # D_u_m / d_uu, which is equal to "self.N" in the same way for each checkpoint.
            self._jac_dict[FEMU_F_KEYS.out_us][FEMU_F_KEYS.inp_us] = np.zeros((self.n_ch_m * self.n_u_m, self.n_ch * self.n_u_ad))
            if self._relative:
                for im in range(self.n_ch_m): # First column of blocks are equal to (-N), due to out_us being relative to the very first checkpoint.
                    si = im * self.n_u_m
                    self._jac_dict[FEMU_F_KEYS.out_us][FEMU_F_KEYS.inp_us][si:si+self.n_u_m, 0:self.n_u_ad] = - self.N[:,:]
                    sj = (im+1) * self.n_u_ad
                    self._jac_dict[FEMU_F_KEYS.out_us][FEMU_F_KEYS.inp_us][si:si+self.n_u_m, sj:sj+self.n_u_ad] = self.N[:,:]
            else:
            ...
            if self.res_reaction_ids_groups != [] and self.res_reaction_ids_groups != [[]]:
                # We extend self.res_reaction_ids_groups for all checkpoints
                import copy
                base_groups = copy.deepcopy(self.res_reaction_ids_groups)
                for i in range(1, self.n_ch):
                    _shift = i * self.n_res
                    _shifted = []
                    for g in base_groups:
                        _shifted.append( [_shift + r for r in g] )
                    self.res_reaction_ids_groups = self.res_reaction_ids_groups + _shifted
            
            J_Fp = np.zeros((self.n_ch*self.n_res, self.n_p)) # D_Residuals / d_pars
            self.J_Fp_splitter = Split2DArray(J_Fp, self.res_reaction_ids_groups)
            self.J_Fu = ConcatenateBlockArray2D(J_Fu_global) # D_Residuals / d_uu
            self.J_Fu_splitter = Split2DArray(self.J_Fu.assembled, self.res_reaction_ids_groups)
                # IMPORTANT: It is essential to do the followings at each call:
                    # self.J_Fp_splitter.A[:,j] = Dgs_dps_checked[0].values(j).flatten()
                    # self.J_Fu.A[:,:] = J_Fu_global
                    # self.J_Fu_splitter.A[:,:] = self.J_Fu.assembled
            
        ##### The operations repeated at each call
        ### VARYING TERMs
        # D_Residuals / d_pars
        for j,v in enumerate(Dgs_dps_checked[0].values()):
            self.J_Fp_splitter.A[:, j] = np.array(v).flatten()
        # D_Residuals / d_uu
        self.J_Fu.A[:,:] = J_Fu_global
        self.J_Fu_splitter.A[:,:] = self.J_Fu.assembled
        
        self._jac_dict[FEMU_F_KEYS.res_reaction][FEMU_F_KEYS.pars] \
            , self._jac_dict[FEMU_F_KEYS.res_other][FEMU_F_KEYS.pars] \
                = self.J_Fp_splitter.split()
        self._jac_dict[FEMU_F_KEYS.res_reaction][FEMU_F_KEYS.inp_us] \
            , self._jac_dict[FEMU_F_KEYS.res_other][FEMU_F_KEYS.inp_us] \
                = self.J_Fu_splitter.split()
        
        return self._jac_dict

...
class FenicsGradientDamage(FenicsProblem, df.NonlinearProblem):
    ...
    def build_solver(self, solver_options=None, time_varying_loadings=[]):
        ...
        if 'newton' in s.lower():
            if bb:
                self.solver = df.NewtonSolver() # A linear solver can calso be specified as input to that.
            else: # 'iterative'
                ...
                lin_solver.parameters['maximum_iterations'] = 2000
                lin_solver.parameters['relative_tolerance'] = 1e-8
                # lin_solver.parameters['absolute_tolerance'] = 1e-14
                # lin_solver.parameters['monitor_convergence'] = True
                # lin_solver.parameters['nonzero_initial_guess'] = True
                
                self.solver = NewtonSolverFromLinearSolver(mpi=self.mesh.mpi_comm(), lin_solver=lin_solver)
                ...
                # self.solver = df.NewtonSolver(self.mesh.mpi_comm(), lin_solver, df.PETScFactory.instance())
            
        elif 'snes' in s.lower():
            if bb:
                self.solver = df.PETScSNESSolver()
            else:
        ...
        self.solver.parameters["absolute_tolerance"] = tol_abs
        self.solver.parameters["relative_tolerance"] = tol_rel
        self.solver.parameters["error_on_nonconvergence"] = allow_nonconvergence_error
        self.solver.parameters["maximum_iterations"] = max_iters
